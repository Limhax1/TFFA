...
public class LeaveCommand extends BaseCommand {
  ...
  public void execute(Player sender) {
    ...
    if (!event.isRunning()) {
      sender.sendMessage(config.getMessage("leave-not-running"));
      return;
    }

    if (!event.getPlayers().contains(sender)) {
      sender.sendMessage(config.getMessage("leave-not-in-event"));
      return;
    }

    event.removePlayer(sender.getPlayer());

    sender.sendMessage(config.getMessage("left-event"));
  }
}
